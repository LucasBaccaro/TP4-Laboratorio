...
int controller_newCopy(char* path, LinkedList* pArrayListEmployee)
{
    int error = -1;
    LinkedList* pArrayListaCopia = ll_clone(pArrayListEmployee);
    ...
    if(pArrayListaCopia != NULL)
    {
        if(ll_containsAll(pArrayListEmployee, pArrayListaCopia) == 1)
        {
            ...
            controller_ListEmployee(pArrayListaCopia);
            ll_deleteLinkedList(pArrayListaCopia);
        }
    }
...
int controller_listSalary(LinkedList* pArrayListSalary)
{
    int todoOk = 0;
    Employee* aux;
    int flag = 0;
    if(pArrayListSalary != NULL)
    {
        printf("ID   NOMBRE   HORAS   SUELDO\n");

        for( int i=0; i < ll_len(pArrayListSalary); i++)
        {
            aux = (Employee*) ll_get(pArrayListSalary,i);
            showOneEmployee(aux);
            flag = 1;
        }
        if(flag == 0)
